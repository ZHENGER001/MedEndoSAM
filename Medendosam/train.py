...
# Function to count the parameters that are fine-tuned
...
if "18" in dataset_name:
    ...
    num_tokens = 2
    val_dataset = VocalfolodsDataset(data_root_dir=data_root_dir, mode="val", vit_mode="h")
    gt_endovis_masks = read_gt_endovis_masks(data_root_dir=data_root_dir, mode="val")
    num_epochs = 600
    lr = 1e-4
    save_dir = "./work_dirs/vocalfolds_1/"
...
# Count the number of fine-tuned parameters
learnable_prototypes_params = count_parameters(learnable_prototypes_model)
protoype_prompt_encoder_params = count_parameters(protoype_prompt_encoder)
sam_decoder_params = count_parameters(sam_decoder)

# Output the fine-tuned parameters count
total_finetuned_params = learnable_prototypes_params + protoype_prompt_encoder_params + sam_decoder_params
print(f"Learnable Prototypes Parameters: {learnable_prototypes_params}")
print(f"Prototype Prompt Encoder Parameters: {protoype_prompt_encoder_params}")
print(f"SAM Decoder Parameters: {sam_decoder_params}")
print(f"Total Finetuned Parameters: {total_finetuned_params}")
...
for epoch in range(num_epochs):
    ...
    if "18" in dataset_name:
        train_dataset = Endovis18Dataset(data_root_dir=data_root_dir, mode="train", vit_mode=vit_mode, version=version)

    elif "vocalfolds" in dataset_name:
        train_dataset = VocalfolodsDataset(data_root_dir=data_root_dir, mode="train", vit_mode=vit_mode, version=version)
