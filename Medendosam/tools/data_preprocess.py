...
# Modified to skip hidden files and non-image files
frame_list = [os.path.join(os.path.basename(subdir), file) 
              for subdir, _, files in os.walk(frame_dir) 
              for file in files 
              if not file.startswith('.') and file.lower().endswith(('.png', '.jpg', '.jpeg'))]

mask_list = [os.path.join(os.path.basename(subdir), file) 
...
for n, frame_name in enumerate(frame_list):
    ...
    if original_frame is None:
        print(f"Warning: Unable to read image {frame_path}. Skipping this frame.")
        continue
    ...
    for version in range(1,n_version+1):
        ...
        feat_save_dir = osp.join(save_dir, f"sam_features_{vit_mode}", frame_name.split(".")[0])
